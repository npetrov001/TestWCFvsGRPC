...
namespace GRPCClient
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            // fix Error "starting gRPC call. HttpRequestException: The HTTP/2 server closed the connection. HTTP/2 error code 'HTTP_1_1_REQUIRED'"
        ...
        private void btnTest_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var channel = GrpcChannel.ForAddress(txtURL.Text.Trim());
                var client = new GrpcTest.GrpcTestClient(channel);

                var input = new PingRequest { Data = "Test" };
                var reply = client.Ping(input);

                TimeSpan ts = PreciseDateTime.Now - dtStart;

                txtOut.Text = String.Format("Time: {0} ms\r\nResponse: {1}", ts.TotalMilliseconds.ToString("#,0.00"), reply.Data);
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            // stop processing
            btnStressStop_Click(this, new EventArgs());
